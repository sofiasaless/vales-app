import { collection, doc, getDoc, getDocs, increment, orderBy, query, Transaction, updateDoc, where } from "firebase/firestore";
...
export class FuncionarioIncentivosFirestore extends PatternFirestore {
  ...
  public async criar(transaction: Transaction, idIncentivo: string, idsFuncionarios: string[]) {
    idsFuncionarios.map((id) => {
      const toSave: FuncionarioIncentivoFirestorePostRequestBody = {
        ...
        ganhador: false,
        incentivo_ref: doc(collection(this.firestore(), COLLECTIONS.INCENTIVO), idIncentivo),
  ...
  public async listar(idIncentivo: string) {
    const snap = await getDocs(
      query(
        this.setup(),
        where("incentivo_ref", "==", doc(collection(this.firestore(), COLLECTIONS.INCENTIVO), idIncentivo)),
